...
fun <T: Any> List<FakeUpdateActions<T>>.getMockList(oldData: List<T>): List<FakeItem<T>> {
...
fun <T: ViewData> DiffUtil.DiffResult.toActionsWithRealIndex(
        oldData: List<T>, newData: List<T>,
        mapChanged: ((FakeOnChanged<T>, List<Int>) -> UpdateActions)? = null): List<UpdateActions> {
    ...
    recorder.fold(oldData.map { it.count })
    { data, act ->
        val nData: List<Int> = when(act) {
            is FakeOnInserted -> {
                val startList = data.subList(0, act.pos)
                val insertListCount = act.insertedItems.map { it.count }
            ...
            is FakeOnRemoved -> {
                val startList = data.subList(0, act.pos)
                val realPos = startList.sum()
                val count = act.removeItems!!.sumBy { it.count }
            ...
            is FakeOnMoved -> {
                if (act.fromPosition < act.toPosition) {
                    ...
                    val secondList = data.subList(act.fromPosition + 1, act.toPosition)

                    val realCount = act.item!!.count
            ...
            is FakeOnChanged -> {
                ...
                val realCountList = act.newItems.map { it.count }
