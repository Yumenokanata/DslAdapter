package indi.yume.tools.dsladapter.updater.list
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.update(data: T, payload: Any? = null): ActionU<ListViewData<T, I, IV>> = { oldVD ->
    val newVD = this.getData(data)

    if (this.keyGetter != null)
        updateAuto(data, diffUtilCheck(this.keyGetter))(oldVD)
    else
        updateVD(oldVD, newVD, payload) to newVD
}

fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.reduce(f: (oldData: T) -> ChangedData<T>): ActionU<ListViewData<T, I, IV>> = { oldVD ->
    val (newData, payload) = f(oldVD.originData)
    update(newData, payload)(oldVD)
}

fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.updateDiffUtil(data: T, keyGetter: KeyGetter<I> = this.keyGetter ?: keyMe()): ActionU<ListViewData<T, I, IV>> =
        { oldVD ->
            val newVD = this.getData(data)
            val actionList = diffUtil(oldVD.data, newVD.data, keyGetter)

            val realActions = if (this.itemIsSingle
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.updateAuto(data: T,
               f: (oldSubs: List<I>, newSubs: List<I>) -> List<UpdateActions> = diffUtilCheck(this.keyGetter ?: keyMe())): ActionU<ListViewData<T, I, IV>> =
        { oldVD ->
            val newVD = this.getData(data)
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.subs(pos: Int, updater: BR.() -> ActionU<IV>): ActionU<ListViewData<T, I, IV>> {
    val subAction = this.subs.updater()
    ...
    return subsAct@{ oldVD ->
        ...
        val newOriData = this.demapper(oldVD.originData, newData)
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.insert(pos: Int, insertedItems: List<I>): ActionU<ListViewData<T, I, IV>> =
        result@{ oldVD ->
            ...
            val insertVD = insertedItems.map { this.subs.getData(it) }
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.remove(pos: Int, count: Int): ActionU<ListViewData<T, I, IV>> =
        result@{ oldVD ->
            if (pos !in 0 until oldVD.data.size
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.move(fromPosition: Int, toPosition: Int): ActionU<ListViewData<T, I, IV>> =
        result@{ oldVD ->
...
fun <T, I, IV : ViewData<I>, BR : BaseRenderer<I, IV>>
        ListRenderer<T, I, IV, BR>.change(pos: Int, newItems: List<I>, payload: Any?): ActionU<ListViewData<T, I, IV>> =
        result@{ oldVD ->
            ...
            val newSubVD = newItems.map { this.subs.getData(it) }

            val newData = oldVD.data.toMutableList().apply {
                for ((index, i) in newItems.withIndex()) set(pos + index, i)
            }.toList()
            val newVD = oldVD.list.toMutableList().apply {
                for ((index, i) in newItems.withIndex()) set(pos + index, this@change.subs.getData(i))
            }.toList()
            val newOriData = this.demapper(oldVD.originData, newData)
